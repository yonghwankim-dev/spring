 ...
 * - verifyThenAssertThat()을 사용하여 검증 이후에 assertion method를 사용하여 추가 검증을 할 수 있다.
 * - hasDiscardedElements()를 사용하여 discard된 데이터가 있는를 검증한다. Backpressure DROP 전략은 Drop된 데이터가 discard된다
...
public class StepVerifierBackpressureTestExample03 {
	...
	void generateNumberTest(){
		StepVerifier
			...
			.hasDiscarded(2, 3, 4, 5, 6, 98, 99, 100); // discard된 데이터가 무엇인지 검증
			// .hasDropped(2, 3, 4, 5, 6, 98, 99, 100); // hasDropped는 Backpressure의 DROP 전략과는 관계없다
	}
}
