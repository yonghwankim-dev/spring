...
class MemberTest {
	...
	public void beforeEach() {
		...
		transaction.begin();
		Member member = new Member(null, "kim", 20);
		Product product = new Product(null, "JPA책");
		em.persist(member);
		em.persist(product);
		em.persist(new Orders(null, member, product, 20000));
		em.flush();
		transaction.commit();
	}
	...
	public void test11() {
		...
		for (Object[] row : resultList) {
			...
			SoftAssertions.assertSoftly(softAssertions -> {
				softAssertions.assertThat(member.getUsername()).isEqualTo("kim");
	...
	public void test12() {
		...
		SoftAssertions.assertSoftly(softAssertions -> {
			softAssertions.assertThat(userDTO.getUsername()).isEqualTo("kim");
	...
	public void test14() {
		...
		transaction.begin();
		for (int i = 0; i < 50; i++) {
			em.persist(new Member(null, "kim" + i, 20 + i));
		}
		transaction.commit();
